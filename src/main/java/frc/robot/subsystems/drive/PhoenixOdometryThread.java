...
public class PhoenixOdometryThread extends Thread {
    ...
    public Queue<Double> registerSignal(StatusSignal<Angle> signal) {
        ...
        signalsLock.lock();
        DrivetrainSim.odometryLock.lock();
        try {
            ...
            signalsLock.unlock();
            DrivetrainSim.odometryLock.unlock();
        }
        return queue;
    }
        ...
        while (true) {
            ...
            try {
                if (isCANFD && phoenixSignals.length > 0) {
                    BaseStatusSignal.waitForAll(2.0 / DrivetrainSim.ODOMETRY_FREQUENCY, phoenixSignals);
                } else {
                    // "waitForAll" does not support blocking on multiple signals with a bus
                    // that is not CAN FD, regardless of Pro licensing. No reasoning for this
                    // behavior is provided by the documentation.
                    Thread.sleep((long) (1000.0 / DrivetrainSim.ODOMETRY_FREQUENCY));
